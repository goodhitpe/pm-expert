...
#### 6교시: 클라우드 인프라 및 PM 관점 정리 (1시간)
- **클라우드 컴퓨팅 서비스 모델**
  - **IaaS(Infrastructure as a Service)**: 가상 서버, 스토리지 (AWS EC2, Azure VM)
  - **PaaS(Platform as a Service)**: 개발 플랫폼 (Heroku, Google App Engine)
  - **SaaS(Software as a Service)**: 완성된 소프트웨어 (Gmail, Slack, Salesforce)
- **클라우드 배포 모델**
  - 퍼블릭 / 프라이빗 / 하이브리드 / 멀티 클라우드
- **온프레미스 vs 클라우드 비교** (PM 관점)
  - 초기 투자 비용, 운영 비용, 확장성, 보안, 관리 부담
